...
class student_attendanceDEL implements ToCollection,WithHeadingRow
{
    /**
    ...
    */
    public function collection(Collection $rows)
    {
        foreach ($rows as $row) 
        {
            $std=StudentAttendance::where('student_id',$row['student_id'])->where('course_code',$row['course_code'])->get();
            if($row['absent']==TRUE)
            {
                if ($row['type']=='lec'){
