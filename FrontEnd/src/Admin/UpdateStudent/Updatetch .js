...
import "./uptea.css"
export default function Updatetch() {
  ...
  useEffect(() =>{
   const res =axios.get(`http://127.0.0.1:8000/api/core/teachingassistant/user-profile/${id1}`,{
     headers:{
...
async function Submit(e){
  e.preventDefault();
  try{
   let res = await axios.post(`http://127.0.0.1:8000/api/core/admin/updateteachingassistant/${id}`,
   {
    id:id,
    full_name:full_name, 
    email:email,
  ...
  async function deleteUser(id){
    console.log(id)
    console.log(token)
        let res=await axios.post(`http://127.0.0.1:8000/api/core/admin/deleteteachingassistant/${id}`,{token},{
        headers:{
          Accept: "application/json",
          Authorization:"Bearer" + token, 
  ...
  return (    
      ...
      <div className="Main-admin">
        ...
        <form className="login-admin"onSubmit={Submit}>
          <p className="headTxt-admin">Update teachingassistant </p>
          ...
          <input type="text" className="id-admin"  required value={id} disabled onChange={(e) => setid(e.target.value)}/><br/><br/>
            ...
            <br/>
            <br/>
          <button type="submit" className="submit99">Update</button>				
